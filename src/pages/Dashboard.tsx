...
import { Play className="w-5 h-5" } from 'lucide-react'; // Make sure to use icons

export default function Dashboard() {
  const user = WebApp.initDataUnsafe?.user?.first_name || 'Спортсмен';
    ...
    <div className="min-h-screen bg-[#121212] text-white p-6 pb-24 font-sans">
      ...
      <div className="flex justify-between items-center mb-8 pt-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">Продолжай в том же духе, {user}!</p>
        </div>
      ...
      {/* Main Today Card */}
      <h2 className="text-xl font-bold mb-4">Тренировка на сегодня</h2>
      <div className="bg-[#1E1E1E] rounded-3xl p-6 border border-[#2C2C2C] shadow-lg relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-6 relative z-10">
          <div>
            <span className="text-[#ADFF00] text-xs font-bold uppercase tracking-wider bg-[#ADFF00]/10 px-3 py-1 rounded-full mb-3 inline-block">FST-7 Фаза 1</span>
            <h3 className="text-2xl font-bold text-white leading-tight">Грудь и <br/>Бицепс</h3>
          </div>
        ...
        <div className="space-y-3 mb-6 relative z-10">
          <p className="text-sm text-gray-400 flex items-center"><span className="w-2 h-2 rounded-full bg-[#ADFF00] mr-2"></span> 6 упражнений</p>
          <p className="text-sm text-gray-400 flex items-center"><span className="w-2 h-2 rounded-full bg-[#ADFF00] mr-2"></span> Высокая интенсивность</p>
        </div>
      ...
      {/* Quick Actions */}
      <h2 className="text-xl font-bold mt-10 mb-4">Действия</h2>
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#1E1E1E] p-5 rounded-2xl border border-[#2C2C2C] active:border-[#ADFF00] transition-colors">
          <h4 className="font-bold mb-1">Дневник Питания</h4>
          <p className="text-xs text-gray-400">ИИ Сканнер еды</p>
        </div>
        <div className="bg-[#1E1E1E] p-5 rounded-2xl border border-[#2C2C2C] active:border-[#ADFF00] transition-colors">
          <h4 className="font-bold mb-1">Прогресс</h4>
          <p className="text-xs text-gray-400">Замеры и фото</p>
        </div>
      </div>
    </div>
  );
