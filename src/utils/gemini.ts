...
const GEMINI_KEY = import.meta.env.VITE_GEMINI_API_KEY || 'AIzaSyBRQTVMtgB6Hdbg7j6U5Y-9y_KgvOFDZn8';
...
export async function analyzeFoodImage(
  ...
  // --- STEP 1: Gemini SDK (direct, like old KINEX) with fast failover ---
  const model = genAI.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: { responseMimeType: 'application/json' },
  });

  // Only 2 retries — fast failover, no wasted API calls
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      console.log(`[gemini.ts] Gemini attempt ${attempt} raw text:`, text.slice(0, 100));
      ...
      if (attempt < 1 && (msg.includes('503') || msg.includes('429') || msg.includes('QUOTA') || msg.includes('UNAVAILABLE') || msg === 'EMPTY_RESPONSE' || msg === 'PARSE_FAILED')) {
        console.warn(`[gemini.ts] Gemini attempt ${attempt} got "${msg}", retrying in ${backoffs[attempt]}ms...`);
        await new Promise(r => setTimeout(r, backoffs[attempt]));
        continue;
      }
      if (msg === 'BAD_PHOTO') throw e;
      console.warn(`[gemini.ts] Gemini failed: ${msg}`);
      break;
    }
  }
